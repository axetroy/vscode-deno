...
export class MockDebugSession extends LoggingDebugSession {
  ...
  public constructor() {
    ...
    this._runtime.on("stopOnBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("breakpoint", MockDebugSession.THREAD_ID)
      );
    });
    this._runtime.on("stopOnDataBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("data breakpoint", MockDebugSession.THREAD_ID)
      );
    });
  ...
  protected setBreakPointsRequest(
    ...
    const actualBreakpoints = clientLines.map((l) => {
      const { verified, line, id } = this._runtime.setBreakPoint(
        path,
